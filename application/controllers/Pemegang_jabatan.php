...
class Pemegang_jabatan extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		if(!$this->session->userdata('id')) redirect('login');
		$this->load->model('pemegang_jabatan_model','model');
	}
	...
	function save()
	{
		...
		if ($data_array['id_pj']) {
			$exec = $this->model->update(['id_pj' => $data_array['id_pj']],$data_array);
		} else{
	...
	function delete()
	{
		...
		$exec = $this->model->delete(['id_pj' => $this->input->post('id')]);
